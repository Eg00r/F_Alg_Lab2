...
def create_conv_model():
    ...
    """
    Здесь создаём свёрточный слой, который имеет 64 фильтра (признака определяемых), размером 16х16 пикселей
    Функция активации - по умолчанию, input_shape - это размерность входных данных, с которыми будем работать.
    На выходе получаем 185х185 для примера (200-16+1).
    """
    
    """Слой уменьшения размера, создаём матрицу 5х5, где берётся максимум каждой области 36х36"""
    ...
    """
    Подсоединяем подследний слой, где количество нейронов равно количеству классов распознаваемых объектов.
    На выходе получаем номер класса, вероятность которого максимальна.
    """
...
if __name__ == '__main__':
    ...
    """Когда создаём новую нейросеть и обучаем её, то раскомментировать нужно две строчки ниже"""
    model = create_conv_model() # Создаём саму модель
    learn_conv_model(model) # Обучаем модель
'''
    """Если хотим работать с уже сохранённой и обученной нейросетью - оставляем только часть ниже"""
   # model = load_model('testIT3.h5') # Загружаем сохранённую модель (если ранее создали, обучили и сохранили в файл)
    ...
    tdir = "test_dog.jpg" # Указываем имя файла, который хотим распознать
    ...
    print("Собака с вероятностью ", res[0][0], ", Кот с вероятностью ", res[0][1])
'''
""" 
Если мы хотим просто пользоваться готовой нейронкой, то нам достаточно содержимого __main__ (11 строчек кода)
Всё остальное нужно только для разработки и обучения.
"""
